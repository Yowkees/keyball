...
## 6. Keybit8の使い方♪ 
   ...
   USB接続直後は通常のキーボードとして使用できるキー入力状態になっています。  
   ...
   各ゲームを終了するには右側4つのキーを同時に長押ししてください。※各ゲームを終了しても引き続きゲームモードの状態は維持され、ゲーム選択待ちになります。 
   ...
   5.Keybit8のリセットスイッチを素早く「カチカチッ」と2回押す。  
   6.画面左上の →（マイコンボードに書き込む） ボタンを押す。
   画面下に「ボードへの書き込みが完了しました」と出れば成功。



   - Arduino IDEの入門としてKeybit8のLED1をチカチカ点滅させ、SW1を押している間はLED1を連続点灯させるコードを記載します。8個のスイッチ、9個のLED、圧電スピーカーのピン番号も記述していますので、どんどんArduino IDEで書き換えて、①LEDを順番に点灯、②８キー全部スキャン、③圧電スピーカーでドレミの音を出す、④ルーレットゲームを作ってみる等、どんどん遊んでみてください。ゲームの力作ができたら、ぜひ公開お願いします。
      ```
      // キースイッチマトリクス（2列×4行）
      ...
      void loop() {
        ...
        if(digitalRead(COL[0]) == HIGH){
          delay(500); //もしSW1が押されていれば500mS待ち
        }
      }
      // delay(500) の数字を変えると点滅の速さが変わるので試してみてください。
      ```
   - Keybit8の回路のピン配置は下記図のようになっています。また、スイッチ番号とLED番号も下記図のような配置になっています。これはKeybit8の基板上の印刷（シルク）での確認できます。緑色発光ダイオード(LED9)と圧電スピーカーはPWM機能対応ピンに接続させているので、パルス幅変調信号を負荷することでLEDの光の明るさを買えたり、スピーカーの音の高さを変えることができます。
      ```text
              　 ProMicroマイコン ピン配置図
                  ┌───────┬──────┬───────┐  
                  │       │ USB  │       │  
       ...
       LED2 ───2──│ 0/RX             GND │──23── GND  
        GND ───3──│ GND   ProMicro   RST │──22── RESET SW                スイッチ番号 配置図
       ...
       COL1 ───8──│ 5~             A0/18 │──17── LED7             ├───────┼───────┼───────┼───────┤     LED番号　配置図
