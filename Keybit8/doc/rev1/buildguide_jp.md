...
## 6. Keybit8の使い方♪ 
   ...
   - Arduino IDEの入門としてKeybit8のLED1をチカチカ点滅させ、SW1を押している間はLED1を連続点灯させるコードを記載します。8個のスイッチ、9個のLED、圧電スピーカーのピン番号も記述していますので、どんどんArduino IDEで書き換えて、①LEDを順番に点灯、②８キー全部スキャン、③圧電スピーカーでドレミの音を出す、④ルーレットゲームを作ってみる等、どんどん遊んでみてください。ゲームの力作ができたら、ぜひ公開お願いします。
      ...
      ```
   - Keybit8の回路のピン配置は下記図のようになっています。また、スイッチ番号とLED番号も下記図のような配置になっています。これはKeybit8の基板上の印刷（シルク）での確認できます。緑色発光ダイオード(LED9)と圧電スピーカーはPWM機能対応ピンに接続させているので、パルス幅変調信号を負荷することで光の明るさを買えたり、音の高さを変えることができます。
      ```text
               　 ProMicroマイコン ピン配置図
                  ┌───────┬──────┬───────┐  
                  │       │ USB  │       │  
