...
static bool trackball_handlers_master(matrix_row_t master_matrix[], matrix_row_t slave_matrix[]) {
    bool trackball_has;
    ...
    trackball_secondary_availablity(trackball_has);
    trackball_delta_t *tb1p = NULL;
    if (trackball_has) {
        tb1p = (trackball_delta_t *)&trackball_delta;
    }
    trackball_apply_delta(1, tb1p);
	
    static uint32_t last_update = 0;
    bool current_trackball_state = trackball_is_on();
    return send_if_condition(PUT_TRACKBALL, &last_update, (current_trackball_state != split_shmem->current_trackball_state), &current_trackball_state, sizeof(current_trackball_state));
}

static void trackball_handlers_slave(matrix_row_t master_matrix[], matrix_row_t slave_matrix[]) {
    // fetch trackball sensor delta and prepare to send it primary.
    trackball_has = trackball_fetch_sensor((trackball_delta_t *)&trackball_delta);
}

#    define TRANSACTIONS_TRACKBALL_MASTER()      TRANSACTION_HANDLER_MASTER(trackball)
#    define TRANSACTIONS_TRACKBALL_SLAVE()       TRANSACTION_HANDLER_SLAVE(trackball)
#    define TRANSACTIONS_TRACKBALL_REGISTRATIONS [PUT_TRACKBALL] = trans_initiator2target_initializer(current_trackball_state),
...
split_transaction_desc_t split_transaction_table[NUM_TOTAL_TRANSACTIONS] = {
	...
	TRANSACTIONS_TRACKBALL_REGISTRATIONS
// clang-format on
