...
const uint16_t MAC_LAYER = 0;
const uint16_t SYMBOL_LAYER = 1;
const uint16_t OS_LAYER = 2;
const uint16_t NUM_LAYER = 3;
const uint16_t MOUSE_LAYER = 4;
    ...
    case SWITCH_LAYER:
      if (record->event.pressed) {
        if (layer_state_is(MAC_LAYER)) {
          layer_on(WIN_LAYER);
        } else if (layer_state_is(WIN_LAYER)) {
          layer_on(GAME_LAYER);
        } else if (layer_state_is(GAME_LAYER)) {
          layer_on(MAC_LAYER);
        }
      }
