...
## 【目次】
  ...
  - [３．実装](#３実装)
    ...
    - [3-6．TRRSソケット・タクトスイッチ・4連ピンソケット](#3-6TRRSソケット・タクトスイッチ・4連ピンソケット)
    - [3-7．ProMicroとOLEDのはんだ付け](#3-7promicroとOLEDのはんだ付け)
    - [3-8．ProMicoの書き込みと動作確認](#3-8promicoの書き込みと動作確認)
  - [４．ボール読み取り基板の組み立て](#４ボール読み取り基板の組み立て)
    - [4-1．センサーのはんだ付け](#4-1センサーのはんだ付け)
  - [５．L字コンスルーのはんだ付け](#５l字コンスルーのはんだ付け)
  - [６．トッププレートのカット（オプション）](#６トッププレートのカット（オプション）)
  - [７．組み立て](#７組み立て)
...
書き込みについてはこの後の[3-8章](#3-8promicoの書き込みと動作確認)でテスト用のファームウェアの書き込み、[１０章](#１０正規ファームウェアの書き込み)で製品版ファームウェアの書き込みについて詳しく説明しています。
...
もし12Pコンスルーの代わりにProMicro用ピンヘッダ＋ピンソケットが同梱されていた場合には、[【こちら】](https://github.com/Yowkees/keyball/blob/main/keyball44/doc/rev1/buildguide_jp.md#3-6promicro%E3%81%AE%E3%81%AF%E3%82%93%E3%81%A0%E4%BB%98%E3%81%91)のビルドガイドをご利用ください。
